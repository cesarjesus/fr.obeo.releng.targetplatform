  ...
  public void checkOptionsOnLocationAreIdentical(final TargetPlatform targetPlatform) {
    ...
    if (_isEmpty) {
      ...
      final Function1<Location, Boolean> _function = new Function1<Location, Boolean>() {
        @Override
        public Boolean apply(final Location __) {
          boolean _isEmpty = Sets.<Option>symmetricDifference(IterableExtensions.<Option>toSet(__.getOptions()), IterableExtensions.<Option>toSet(first.getOptions())).isEmpty();
      ...
      if (_not) {
        final Consumer<Location> _function_1 = new Consumer<Location>() {
          ...
          public void accept(final Location __) {
            final List<INode> nodes = NodeModelUtils.findNodesForFeature(__, TargetPlatformPackage.Literals.LOCATION__OPTIONS);
            ...
            if (_not) {
              ...
              int _offset_1 = withKeyword.getOffset();
              int _minus = (_endOffset - _offset_1);
              TargetPlatformValidator.this.acceptError("Options of every locations must be the same", __, _offset, _minus, TargetPlatformValidator.CHECK__OPTIONS_EQUALS_ALL_LOCATIONS);
            } else {
              final ICompositeNode node = NodeModelUtils.getNode(__);
              TargetPlatformValidator.this.acceptError("Options of every locations must be the same", __, node.getOffset(), node.getLength(), TargetPlatformValidator.CHECK__OPTIONS_EQUALS_ALL_LOCATIONS);
            }
          }
        };
